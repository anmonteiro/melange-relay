...
module Test = {
  ...
  let make = () => {
    ...
    let pluralNonNullCount =
      dataPlural
      ->(Belt.Array.keep(item => item->Belt.Option.isSome))
      ->Belt.Array.length;
    let pluralNullCount =
      dataPlural
      ->(Belt.Array.keep(item => item->Belt.Option.isNone))
      ->Belt.Array.length;
    ...
    <div>
      ...
      <div className="plural">
        <div>
          {React.string(
             "Non null count: " ++ Js.Int.toString(pluralNonNullCount),
           )}
        </div>
        <div>
          {React.string("Null count: " ++ Js.Int.toString(pluralNullCount))}
        </div>
        <div>
          {React.string(
             "Users: "
